  ...
  return (
    <div className="animate-fade-in w-full flex flex-col items-center justify-center space-y-10 h-full pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-2xl">
        {[
          { q: "What was total revenue and net income?", label: "Financials" },
          { q: "What AI-related risk factors were disclosed?", label: "Risk Factors" },
          { q: "How did operating margin change YoY?", label: "Margins" },
          { q: "What did management say about future guidance?", label: "Outlook" },
        ].map((item, i) => (
          <button
            key={i}
            onClick={() => onQuickSelect(item.q)}
            className="flex flex-col gap-1 p-4 text-left border border-border rounded-xl bg-transparent hover:bg-muted/40 hover:border-primary/50 transition-all shadow-sm hover:shadow-[0_0_15px_rgba(0,255,255,0.1)] group"
          >
            <div className="text-sm font-bold text-primary mb-1 group-hover:text-primary transition-colors">{item.label}</div>
            <div className="text-sm font-medium text-foreground leading-snug">{item.q}</div>
          </button>
        ))}
      </div>
    </div>
...
export default function QueryPanel({
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative bg-transparent">
      ...
      <div className="w-full flex-shrink-0 px-4 sm:px-6 pb-4 sm:pb-6 pt-4 bg-transparent border-t border-border/10">
